    ...
    for i in range(n):
        ...
        while not_duplicates_found == False:
            try:
                ...
                name = input_valid_name("agregar")
                section = input_valid_section("agregar")
                if find_student(name,section,data)[1] == False:
        ...
        spanish = input_valid_grade("Español")
        english = input_valid_grade("Inglés")
        socials = input_valid_grade("Estudios Sociales")
        sciences = input_valid_grade("Ciencias")
        data.append(Student(name,section,spanish,english,socials,sciences))
...
def show_info(data):
    for student in data:
        ...
        for asignature in indexed_asignatures:
            print(f" -{asignature['esp']}: {getattr(student, asignature['eng'])}")
...
def show_failed_students(data):
    ...
    for student in data:
        for asignature in indexed_asignatures:
            if float(getattr(student,asignature['eng'])) < 60:
...
def calculate_avg(data):
    for index, student in enumerate(data):
        total_sum = 0
        for asignature in indexed_asignatures:
            total_sum += float(getattr(student, asignature['eng']))
        data[index].avg = round((total_sum/4),2)
    return data

def order_by_avg(data):
    return sorted(data, key = lambda student: student.avg, reverse = True)
